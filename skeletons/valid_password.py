...
assert all(
    passes_length(pwd) == ans
    for pwd, ans in zip(
        ["acd748ghc", "1yd758gja1", "473hdjqudhfgn"], [False, True, True]
    )
), "passes_length failed"
print("passes_length passed")

assert all(
    passes_special_symbols(pwd) == ans
    for pwd, ans in zip(
        ["acd8ghc", "1yd7_Bgja1", "$", "7f!67@2G"], [False, True, True, True]
    )
), "passes_special_symbols failed"
print("passes_special_symbols passed")

assert all(
    passes_capital_chars(pwd) == ans
    for pwd, ans in zip(
